...
print('Артём мог загадать следующие числа: ',
      *full_set)  # * - оператор раскрытия (в данном контексте), т. е. если мы передаём в функцию список без звёздочки, то ей передаётся список целиком, как один аргумент, а со звёздочкой каждый элемент будет отдельно.
